...
/** Menu of actions, e.g. for a table row (shadcn/ui, adapted to the GenTouch tokens). */

const itemClassName =
  'relative flex cursor-default items-center gap-2 rounded-md px-2 py-2 text-sm outline-hidden select-none focus:bg-accent focus:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*=size-])]:size-4';
...
function DropdownMenuItem({
  ...
  return (
    <DropdownMenuPrimitive.Item
      ...
      className={cn(
        itemClassName,
        'data-[inset]:pl-8 data-[variant=destructive]:text-error data-[variant=destructive]:focus:bg-error-tint data-[variant=destructive]:focus:text-error [&_svg:not([class*=text-])]:text-muted',
        className,
      )}
      {...props}
    />
...
function DropdownMenuCheckboxItem({ className, children, checked, ...props }: ComponentProps<typeof DropdownMenuPrimitive.CheckboxItem>) {
  return (
    <DropdownMenuPrimitive.CheckboxItem data-slot="dropdown-menu-checkbox-item" className={cn(itemClassName, 'pl-8', className)} checked={checked} {...props}>
...
function DropdownMenuRadioItem({ className, children, ...props }: ComponentProps<typeof DropdownMenuPrimitive.RadioItem>) {
  return (
    <DropdownMenuPrimitive.RadioItem data-slot="dropdown-menu-radio-item" className={cn(itemClassName, 'pl-8', className)} {...props}>
      <span className="pointer-events-none absolute left-2 flex size-4 items-center justify-center">
        <DropdownMenuPrimitive.ItemIndicator>
...
function DropdownMenuLabel({ className, inset, ...props }: ComponentProps<typeof DropdownMenuPrimitive.Label> & { inset?: boolean }) {
  return (
    <DropdownMenuPrimitive.Label
      ...
      data-inset={inset}
      className={cn('px-2 py-1.5 text-xs font-semibold tracking-wider text-muted uppercase data-[inset]:pl-8', className)}
      {...props}
    />
  );
}
...
function DropdownMenuShortcut({ className, ...props }: ComponentProps<'span'>) {
  return <span data-slot="dropdown-menu-shortcut" className={cn('ml-auto text-xs tracking-widest text-muted', className)} {...props} />;
}
...
function DropdownMenuSubTrigger({ className, inset, children, ...props }: ComponentProps<typeof DropdownMenuPrimitive.SubTrigger> & { inset?: boolean }) {
  return (
    <DropdownMenuPrimitive.SubTrigger
      ...
      data-inset={inset}
      className={cn(itemClassName, 'data-[inset]:pl-8 data-[state=open]:bg-accent data-[state=open]:text-accent-foreground', className)}
      {...props}
    >
      {children}
...
function DropdownMenuSubContent({ className, ...props }: ComponentProps<typeof DropdownMenuPrimitive.SubContent>) {
  return (
    <DropdownMenuPrimitive.SubContent
      ...
      className={cn('origin-(--radix-dropdown-menu-content-transform-origin)', contentClassName, className)}
      {...props}
    />
  );
}
