...
Known M5 issues, to fix in 5.6 (deployment):
- **Sign-in links on shared connections.** Better Auth's magic-link plugin allows 5 requests per IP address, to request a link and to open it, and the count clears only after a quiet minute. Leaders on the same church Wi-Fi, or behind a mobile carrier's shared address, share that budget: at a leaders' onboarding session, the sixth person to request or open a link within a minute of the others is refused ("Too many attempts", or a bare error page for the link itself). Proposed: a higher per-IP limit for these two routes, plus an app-level limit per email address so no one's inbox can be flooded.
- **Rate-limit storage in production.** Better Auth keeps its counts in memory by default, which on Vercel means per serverless instance: limits aren't enforced consistently. Proposed: back it with the Upstash Redis the app already uses for its own limits.
