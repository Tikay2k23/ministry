...
Known M5 gaps, planned for later:
- The public journal’s first load (LCP on a low-end phone over slow 4G, and the 90 KB JavaScript budget) is checked by hand, not automatically.
- The k6 load tests need a staging environment; they are written (`load/`) but not run on a schedule yet.
- The manual screen-reader pass (`docs/accessibility-checklist.md`) is still to be run before the pilot.
- Journal photo proof: the Supabase Storage driver is written but untested until the project exists, and the retention rule for photos is still the ministry’s to decide (docs/runbooks/recovery.md). Requiring a photo will lower completion on cheap phones and slow connections — worth watching in the pilot, and one setting to relax.
